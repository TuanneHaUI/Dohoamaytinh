...
void matdongho()
{
	...
	instance = model_mat_cpp * scale(vec3(0.6, 0.6, 0.04));
	glUniformMatrix4fv(model_mat_location, 1, GL_FALSE, instance.m);
	glDrawArrays(GL_TRIANGLES, 0, 36);	
	
}
// ------------------------------------------
//Hàm vẽ kim cho dong ho
void kimgio()
{
	mat4 instance = identity_mat4();
	instance = model_mat_cpp * scale(vec3(0.2, 0.06, 0.06));
	glUniformMatrix4fv(model_mat_location, 1, GL_FALSE, instance.m);
	glDrawArrays(GL_TRIANGLES, 0, 36);		
}
void kimphut()
{
	mat4 instance = identity_mat4();
	instance = model_mat_cpp * scale(vec3(0.06, 0.25, 0.06));
	glUniformMatrix4fv(model_mat_location, 1, GL_FALSE, instance.m);
	glDrawArrays(GL_TRIANGLES, 0, 36);
}
//ve vien cho dong ho
...
{
	mat4 instance = identity_mat4();
	instance = model_mat_cpp * scale(vec3(0.02, 0.6, 0.06));
	glUniformMatrix4fv(model_mat_location, 1, GL_FALSE, instance.m);
	glDrawArrays(GL_TRIANGLES, 0, 36);
}
void vienngang()
{
	mat4 instance = identity_mat4();
	instance = model_mat_cpp * scale(vec3(0.6, 0.02, 0.06));
	glUniformMatrix4fv(model_mat_location, 1, GL_FALSE, instance.m);
	glDrawArrays(GL_TRIANGLES, 0, 36);
}
// ---------------------------------------------------------------------------	
void DisplayFunc(void)
{
	...
	matdongho();
	model_mat_cpp = rotate_y(Alpha_2) * rotate_x(Alpha_1) * translate(vec3(T[0], T[1], T[2]))*translate(vec3(-0.0, 0.29, -0.05));
	vienngang();
	model_mat_cpp = rotate_y(Alpha_2) * rotate_x(Alpha_1) * translate(vec3(T[0], T[1], T[2]))*translate(vec3(-0.0, -0.29, -0.05));
	vienngang();
	model_mat_cpp = rotate_y(Alpha_2) * rotate_x(Alpha_1) * translate(vec3(T[0], T[1], T[2]))* translate(vec3(-0.29, 0.0, -0.05));
	viendoc();
	model_mat_cpp = rotate_y(Alpha_2) * rotate_x(Alpha_1) * translate(vec3(T[0], T[1], T[2]))* translate(vec3(0.29, 0.0, -0.05));
	viendoc();
	model_mat_cpp = rotate_z(Alpha_3/12) * rotate_y(Alpha_2) * rotate_x(Alpha_1) * translate(vec3(T[0], T[1], T[2]))* translate(vec3(-0.1, -0.0, -0.1));
	kimgio();
	model_mat_cpp = rotate_z(Alpha) * rotate_y(Alpha_2) * rotate_x(Alpha_1) * translate(vec3(T[0], T[1], T[2]))* translate(vec3(-0.02, 0.115, -0.1));
	kimphut();
